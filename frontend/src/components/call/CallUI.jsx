/**
 * CallUI.jsx — WebRTC P2P Call Interface
 *
 * Features:
 *   ✦ Video tiles (local PIP + remote full-screen)
 *   ✦ Connection quality badge (RTT / packet-loss / bitrate)
 *   ✦ DataChannel in-call chat panel
 *   ✦ Device switcher (camera + mic dropdowns)
 *   ✦ Screen sharing indicator
 *   ✦ Reconnecting overlay
 *   ✦ Mobile-safe layout (no overlapping controls)
 */
import { useState, useRef, useEffect, useCallback } from 'react';

/* ── Animated SVG Icons (no external icon library needed) ───── */
const Ico = {
...
function ReconnectingOverlay({ attempt }) {
  return (
    <div style={styles.reconnectOverlay}>
      ...
      {attempt > 0 && (
        <p style={styles.reconnectSub}>Attempt {attempt} / 3</p>
      )}
    </div>
  );
}
...
function DevicePanel({ availableDevices, onSwitchCamera, onSwitchMicrophone, onClose }) {
  return (
    <div style={styles.devicePanel}>
      ...
      <p style={{ ...styles.chatEmpty, textAlign: 'left', paddingTop: 8 }}>
        🔒 Switching happens without renegotiation
      </p>
    </div>
  );
}
        ...
        <div style={styles.topBar}>
          <div style={styles.callInfo}>
            ...
            <span style={styles.callStatus}>
              {isReconnecting ? '↺ Reconnecting'
                : isConnecting ? 'Connecting…'
                : callState?.status === 'active' ? 'P2P Encrypted'
                : ''}
            </span>
          </div>
...
/* ══════════════════════════════════════════════════════════════
   Styles — inline JS objects (no Tailwind / CSS module needed)
══════════════════════════════════════════════════════════════ */
const styles = {
  root: {
    position: 'fixed',
...
if (typeof document !== 'undefined' && !document.getElementById('callui-spinner-anim')) {
  const style = document.createElement('style');
  style.id = 'callui-spinner-anim';
