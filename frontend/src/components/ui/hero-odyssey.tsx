...
export interface HeroSectionProps {
  /** Override the main heading */
  title?: string;
  /** Override the sub-heading */
  subtitle?: string;
  ...
  /** Footer tech tags */
  techTags?: string[];
  /** Feature callout items (max 4) */
  features?: Array<{ name: string; value: string }>;
  /** Initial lightning hue (0-360) */
  defaultHue?: number;
  /** Nav links displayed in top bar */
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  title = "SmartChat X",
  subtitle = "Lighting Up The Future",
  description = "Next-gen communication powered by WebRTC, Groq AI, and dual-protocol smart routing. Encrypted, fast, intelligent.",
  ...
  features = [
    ...
    { name: "Framer Motion", value: "animations" },
  ...
  const featurePositions = [
    "left-0 sm:left-10 top-40",
    "left-1/4 top-24",
    "right-1/4 top-24",
    "right-0 sm:right-10 top-40",
  ];
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 h-screen">
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col items-center justify-center h-full space-y-6 text-lg">
              ...
              <button className="px-6 py-3 bg-gray-800/50 rounded-full" onClick={onPrimaryCta}>Get Started</button>
              {navLinks.map((link) => (
                <button key={link.label} className="px-6 py-3">{link.label}</button>
              ))}
        ...
        >
          {features.slice(0, 4).map((feat, i) => (
            <motion.div key={feat.name} variants={itemVariants}>
              <FeatureItem name={feat.name} value={feat.value} position={featurePositions[i]} />
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          ...
          >
            <span className="w-1.5 h-1.5 bg-white rounded-full inline-block" style={{ boxShadow: '0 0 8px #fff' }} />
            <span>{badgeText}</span>
          ...
          <motion.div variants={itemVariants} className="flex gap-4 flex-wrap justify-center">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={onPrimaryCta}
              className="px-8 py-3 bg-white/90 text-black font-semibold rounded-full hover:bg-white transition-colors flex items-center gap-2"
            >
              {primaryCta}
          ...
          <motion.p
            variants={itemVariants}
            style={{ marginTop: 56, fontSize: 11, fontFamily: 'monospace', color: 'rgba(255,255,255,0.18)', letterSpacing: '0.15em', textTransform: 'uppercase' }}
          >
            {techTags.join(' · ')}
          </motion.p>
        </motion.div>
